...
class MyPlayer(PlayerHex):
    ...
    def compute_action(
        ...
        **kwargs,
        ...
        # --- Ladder detection ---
        # 1. Can we ladder the opponent? → play first move
        ladder_attack = self._detect_ladder_attack(board, my_piece, geom)
        if ladder_attack is not None:
            return self._to_action(ladder_attack, n)

        # 2. Can the opponent ladder us? → play escape
        ladder_escape = self._detect_ladder_escape(board, my_piece, geom)
        if ladder_escape is not None:
            return self._to_action(ladder_escape, n)
    ...
    def _simulate_ladder_path(self, board, start, piece, direction_idx, geom):
        """Simule un zigzag forcé depuis `start` dans une direction.

        direction_idx: 0 = toujours le premier voisin forward, 1 = toujours le second.
        Retourne le chemin complet (liste d'indices) si le ladder atteint le bord, sinon None.
        """
        n = geom["n"]
        rows = geom["rows"]
        cols = geom["cols"]
        ...
        if piece == "R":
            forward_deltas = [(1, -1), (1, 0)]
            goal = lambda idx: rows[idx] >= n - 1
        else:
            forward_deltas = [(-1, 1), (0, 1)]
            goal = lambda idx: cols[idx] >= n - 1
        ...
        for _ in range(n * 2):
            ...
            if goal(current) and path:
                return path

            # Deux voisins forward
            fwd = []
            for dr, dc in forward_deltas:
                nr, nc = r + dr, c + dc
                if 0 <= nr < n and 0 <= nc < n:
                    fwd.append(nr * n + nc)
                else:
                    return None  # Hors plateau

            if len(fwd) != 2:
                return None

            # Les deux doivent être vides
            if sim[fwd[0]] != "." or sim[fwd[1]] != ".":
                return None

            # Attaquant joue fwd[direction_idx], défenseur bloque l'autre
            play = fwd[direction_idx]
            block = fwd[1 - direction_idx]
    ...
    def _detect_ladder_attack(self, board, piece, geom):
        """Cherche un ladder gagnant pour `piece`. Retourne le premier coup ou None."""
        n = geom["n"]
        rows = geom["rows"]
        cols = geom["cols"]
        ...
        for start in range(len(board)):
            ...
            # Ne tenter que depuis les pièces proches de la frontière
            if piece == "R":
                # Rouge va de haut en bas — tenter depuis les pièces pas encore au bord
                if rows[start] >= n - 1:
                    continue
            else:
                if cols[start] >= n - 1:
            ...
            # Essayer les deux directions de zigzag
            for d in [0, 1]:
                path = self._simulate_ladder_path(board, start, piece, d, geom)
                if path is not None and len(path) >= 1:
                    return path[0]

        return None

    def _detect_ladder_escape(self, board, piece, geom):
        """Détecte si l'adversaire peut nous ladder. Si oui, retourne le meilleur coup de blocage.

        Le blocage est placé sur le chemin du ladder adverse, à la position
        avec le meilleur v(1-v) dans notre réseau (utile pour nous ET bloque le ladder).
        """
        n = geom["n"]
        opp = _other(piece)

        # Chercher tous les ladders adverses
        ladder_paths = []
        for start in range(len(board)):
            if board[start] != opp:
                continue
            if opp == "R" and geom["rows"][start] >= n - 1:
                continue
            if opp == "B" and geom["cols"][start] >= n - 1:
                continue

            for d in [0, 1]:
                path = self._simulate_ladder_path(board, start, opp, d, geom)
                if path is not None:
                    ladder_paths.append(path)

        if not ladder_paths:
            return None

        # Collecter toutes les cases sur les chemins de ladder
        ladder_cells = set()
        for path in ladder_paths:
            for idx in path:
                if board[idx] == ".":
                    ladder_cells.add(idx)

        if not ladder_cells:
            return None

        # Choisir la case avec le meilleur v(1-v) dans notre réseau
        board_key = tuple(board)
        my_voltages = self._voltage_cache.get((piece, board_key))
        if my_voltages is None:
            self._calculate_amperage(board_key, piece, geom)
