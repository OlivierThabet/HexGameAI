...
def _get_template_II_instances(n: int, piece: str):
    ...
    insts = []
    if piece == "R":
        # Top edge: stone at (1,c), carrier at (0,c) and (0,c+1)
        for c in range(n - 1):
            insts.append((1 * n + c, 0 * n + c, 0 * n + (c + 1)))
        # Bottom edge: stone at (n-2,c), carrier at (n-1,c) and (n-1,c-1)
        for c in range(1, n):
            insts.append(((n - 2) * n + c, (n - 1) * n + c, (n - 1) * n + (c - 1)))
    else:
        # Left edge: stone at (r,1), carrier at (r,0) and (r+1,0)
        for r in range(n - 1):
            insts.append((r * n + 1, r * n, (r + 1) * n))
        # Right edge: stone at (r,n-2), carrier at (r,n-1) and (r-1,n-1)
        for r in range(1, n):
...
def _get_template_III_instances(n: int, piece: str):
    ...
    key0 and key1 are the two cells on the intermediate row (row 1).
    """
    insts = []
    ...
    if piece == "R":
        for c in range(n):
            s = _cell(2, c, n)
            # Variant A: keys at (1,c) and (1,c+1), rest on row 0
            _try(s, [_cell(1, c, n), _cell(1, c+1, n),
                      _cell(0, c, n), _cell(0, c+1, n), _cell(0, c+2, n)],
                 _cell(1, c, n), _cell(1, c+1, n))
            # Variant B (mirror): keys at (1,c-1) and (1,c), rest on row 0
...
def _get_template_IV_instances(n: int, piece: str):
    ...
    Returns (stone_idx, all_carrier, key0, key1).
    key0 and key1 are the two cells on row 2.
    """
    insts = []
...
def _find_double_bridges(board: List[str], piece: str, geom: dict) -> List[Tuple[int, int]]:
    """
    Find pairs of friendly stones connected by 2+ bridges with
    non-overlapping carriers. Unbreakable interior connection.
    """
    opp = _other(piece)
...
class MyPlayer(PlayerHex):
    ...
    def compute_action(self, current_state: GameStateHex,
        ...
        # If opponent has a template II or IV on a stone that carries
        # significant current (on their main path), and the greedy search
        # didn't pick a move that breaks it, override.
    ...
    def _greedy_pick(self, board, empties, my_piece, geom):
        """Return best move index (not action)."""
    ...
    # -----------------------------------------------------------------------
    # Electrical model + bridges + templates II/III/IV + double bridges
    # -----------------------------------------------------------------------
    ...
    def _calculate_amperage(self, board_key, target_piece, geom):
        ...
        res_map = {target_piece: 0.001, ".": 1.0, opp: float("inf")}
        ...
        # Contact resistance to prevent boundary singularities
        R_CONTACT = 1.0 
        ...
        x0 = self._last_voltages.get(direction)
        ...
        try:
            ...
            self._last_voltages[direction] = voltages
            ...
            for i in range(n):
                ...
                idx = row * n + col
                if resistances[idx] != float("inf"):
                    # Calculate final current exiting the source using the same R_CONTACT
    ...
    def _ordered_moves(self, board, moves, piece, geom):
        ...
        for idx in moves:
            ...
            # Combine: building our path + disrupting opponent's path
    ...
    def _critical_template_override(self, board, chosen_move, empties,
        ...
        """
        After greedy/minimax picked a move, check if the opponent has
        a template IV on a stone that's on their main current path.
        If so, and the chosen move doesn't already break it, override.

        Template II and III are unbreakable — no point blocking.
        Template IV is breakable — intrude on a key cell (row 2).

        Only triggers when the template stone has high current flow
        (voltage between 0.2 and 0.8), meaning it's on the main path.

        Returns override move index, or None if no override needed.
        """
        n = geom["n"]
        board_key = tuple(board)
        ...
        # --- Check opponent template IV only ---
        ...
        # Check if the chosen move already breaks a critical template
        chosen_set = {chosen_move}
        for _, target in critical_targets:
            if target in chosen_set:
                return None  # already blocking

        # Sort by current density (highest first) and pick the best
        # blocking move by evaluation
        ...
        # Evaluate: only override if the blocking move isn't much worse
        # than the chosen move
        ...
        for mv in candidates[:4]:  # check top 4 at most
            board[mv] = my_piece
        ...
        # Only override if the blocking move isn't catastrophically worse
        # (allow up to 20% amperage loss to block a critical template)
