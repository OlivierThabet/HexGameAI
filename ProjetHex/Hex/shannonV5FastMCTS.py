'''
A partir de shannonV5FastMiniMax,
La version MCTS remplace l'alpha-bêta par une recherche arborescente Monte Carlo. Au lieu d'explorer exhaustivement un arbre de profondeur fixe, MCTS construit l'arbre de façon asymétrique en répétant quatre étapes en boucle jusqu'à épuisement du temps : sélection d'un nœud prometteur via UCB1, expansion d'un enfant non visité, simulation (rollout), et rétropropagation du résultat.
La différence clé est le rollout. Plutôt que des parties aléatoires complètes (trop bruitées sur Hex), on joue 6 coups semi-guidés — choisis aléatoirement parmi le top-3 de l'ordonnancement heuristique — puis on évalue la position avec l'ampérage. Le score est normalisé en [0,1] via une sigmoid, car MCTS accumule des moyennes de récompenses contrairement à MiniMax qui compare des scores absolus.
Le choix final diffère aussi : MiniMax retourne le coup avec le meilleur score, MCTS retourne le coup le plus visité, ce qui est plus robuste car un coup beaucoup visité a été confirmé par de nombreuses simulations.
Tout le reste est strictement identique — modèle électrique, bridges virtuels, ordonnancement, ouverture, TOP_K — ce qui garantit que la seule variable expérimentale est l'algorithme de recherche, rendant la comparaison valide pour le rapport.
'''
...
class MyPlayer(PlayerHex):
    ...
    EXPLORATION = 1.414
    ROLLOUT_DEPTH = 6           # Profondeur max du rollout avant évaluation heuristique
    ...
    def compute_action(
        ...
        **kwargs,
        ...
        # Limiter les coups candidats avec le même ordonnancement heuristique
        ...
        while time.time() < deadline:
            ...
            # 3. SIMULATION — rollout avec heuristique
    ...
    def _rollout(
        ...
        # Rollout semi-aléatoire de profondeur limitée
        for _ in range(self.ROLLOUT_DEPTH):
            ...
            # Choisir un coup : top-3 de l'ordonnancement, puis aléatoire parmi eux
        ...
        # Évaluation heuristique à la fin du rollout
        ...
        # Normaliser en [0, 1] avec sigmoid
        diff = my_amp - opp_amp
        return 1.0 / (1.0 + math.exp(-0.05 * diff))

    # -----------------------------------------------------------------------
    # Modèle électrique (identique à la version MiniMax)
    # -----------------------------------------------------------------------
