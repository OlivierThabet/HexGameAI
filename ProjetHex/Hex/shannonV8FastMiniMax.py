'''
On enleve le bonus à coller les murs.
'''
...
class MyPlayer(PlayerHex):
    ...
    def _ordered_moves(self, board: List[str], moves: List[int], piece: str, geom: dict) -> List[int]:
        ...
        for idx in moves:
            ...
            # Centralité : les positions centrales sont les plus fortes à Hex
            dist_center = abs(row - center) + abs(col - center)
            center_score = -0.5 * dist_center

            # Bridge construction : bonus pour les coups à distance de bridge
            ...
            # Prise d'espace : non-adjacent mais relié par bridge = idéal
            reach_bonus = 0.0
            if own_n == 0:
                for partner, c1, c2 in geom["bridge_links"][idx]:
                    if board[partner] == piece:
