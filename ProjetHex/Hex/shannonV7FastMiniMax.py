'''
A partir de ShannonV6FastMiniMax, on change les points de recompenses. Maintenant on cherche a setendre.'''
    ...
    for idx in range(len(board)):
        ...
        for partner, c1, c2 in geom["bridge_links"][idx]:
            if board[partner] != opp:
                continue
            # Bridge adverse intact : les deux intermédiaires sont libres
...
class MyPlayer(PlayerHex):
    ...
    def _ordered_moves(self, board: List[str], moves: List[int], piece: str, geom: dict) -> List[int]:
        ...
        # Rouge connecte haut-bas (lignes), Bleu connecte gauche-droite (colonnes)
        my_min = n
        my_max = -1
        ...
        for idx in moves:
            ...
            # Bonus fort pour les coups qui étendent notre portée
            ...
            # --- Bridge / espace : bonus pour les coups à distance de bridge ---
            ...
            # --- Reach bonus : récompenser les coups qui ne sont PAS adjacents ---
            # mais qui sont à distance de bridge d'une pièce alliée (prise d'espace)
            reach_bonus = 0.0
            if own_n == 0:
                for partner, c1, c2 in geom["bridge_links"][idx]:
                    if board[partner] == piece:
            ...
            score = (
                1.0 * own_n           # adjacence réduite (était 3.2)
                + 2.0 * opp_n         # bloquer l'adversaire reste important
                + secondary_center    # rester centré sur l'axe secondaire
                + extend_bonus        # FORT : étendre la portée
                + edge_bonus          # bonus proximité bords cibles
                + reach_bonus         # bonus prise d'espace via bridge
                + bridge_bonus
                + 0.1 * hist
            )
